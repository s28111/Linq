...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Join(Emps,dept => dept.Deptno, emp => emp.Deptno, (emp,dept) => new
            {
                Nazwisko = emp.Ename,
                Praca = emp.Job,
                Dzial = dept.Ename
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(emp => emp.Job).Select( dept => new
            {
                Praca = dept.Key,
                LiczbaPracownikow = dept.Count()
            });
            return result;
        }
        ...
        {
            bool result = false;
            IEnumerable<object> allJobs = Emps.Select(emp => emp.Job);
            result = allJobs.Contains("Backend programmer");
            return result;
        }
        ...
        {
            Emp result = Emps.Where(emp => emp.Job == "Fronted programmer")
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            {
                new {Ename = "Brak wartości", Job = (string)null,HireData = (DateTime?) null}
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(emp => emp.Deptno).Where(group => group.Count() > 1).Select(
                group => new
                {
                    name = group.Key,
        ...
        {
            IEnumerable<Emp> result = CustomExtensionMethods.GetMenagers(Emps);
            return result;
        }
        ...
        {
            int result = 0;
            result = arr.GroupBy(i => i).Where(group => group.Count() % 2 == 1).Max(group => group.Key);
            return result;
        }
        ...
        public static IEnumerable<object> Task15()
        {
            ...
            result = Emps.Where(emp => emp.Job.Contains("A")).GroupBy(emp => emp.Job).Where(group => group.Count() > 2)
        ...
        {
            IEnumerable<object> result = null;
            result = Depts.SelectMany(dept => Emps, (dept, emp) => new { emp, dept });
            return result;
        }
    }
    ...
    {
        public static IEnumerable<Emp> GetMenagers(this IEnumerable<Emp> emps)
        {
            var result = emps.Where(emp => emps.Any(e => e.Mgr != null && e.Mgr.Empno == emp.Empno)).Distinct()
